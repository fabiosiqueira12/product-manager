...
function MaskMonetario(v) {
    v = parseFloat(v).toFixed(2).toString().replace(/\D/g, ""); /* Remove tudo o que nÃ£o Ã© dÃ­gito */
...
function mask_full(valor, mascara) {
    ...
    valorm = valor;
    /* tirando mascara do valor jÃ¡ existente */
    ...
    if (temp.length == 1) {
        ...
        /* limpar caracteres diferente do caracter da mÃ¡scara */
        for (i = 0; i < temp.length; i++) {
            mascara_limpa = "";
            for (j = 0; j < temp[i].length; j++) {
